...
export async function GET(request: NextRequest) {
  try {
    ...
    // No call in database - just report presence status (don't create sessions here)
    // Sessions should be created by the 3CX WebSocket bridge webhook
    if (isOnCallPerVoIP) {
      console.log(`[Active Calls] VoIPTools shows active call for ${extension}, but no DB session yet (waiting for webhook)`);
      return NextResponse.json({
        success: true,
        hasActiveCall: true,
        calls: [{
        ...
        source: "voiptools",
        message: "Call detected via presence - waiting for WebSocket bridge to create session",
      });
    }
