...
import { eq, and, sql, or, ilike, gte, lte, lt, isNull, isNotNull } from "drizzle-orm";
import {
  fetchNewRecordings,
...
async function matchExistingCall(callData: ReturnType<typeof mapRecordingToCallData>) {
  ...
  // Priority: find a Twilio-anchored call (no 3CX recording yet) by phone+time
  if (phone) {
    const phoneSuffix = `%${phone.slice(-10)}`;

    const [twilioAnchor] = await db
      .select({ id: calls.id })
      .from(calls)
      .where(and(
    ...
    if (twilioAnchor) {
      console.log(`[recording-poller] Matched Twilio anchor ${twilioAnchor.id}`);
      return twilioAnchor;
    }
  }
