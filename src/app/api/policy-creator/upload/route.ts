...
const EXTRACTION_PROMPT = `You are an expert insurance document analyzer. Analyze this insurance dec page or policy application image and extract all relevant policy information.
 ...
 * Claude supports PDFs natively as document types.
 */
function pdfToBase64(pdfBuffer: ArrayBuffer): string {
 ...
 */
async function extractWithClaudePDF(
  pdfBase64: string
...
function normalizeCoverage(raw: ExtractedPolicyData['coverages']): CanonicalCoverage[] {
  if (!raw) return [];
...
function normalizeVehicles(raw: ExtractedPolicyData['vehicles']): CanonicalVehicle[] {
  if (!raw) return [];
...
function normalizeDrivers(raw: ExtractedPolicyData['drivers']): CanonicalDriver[] {
  if (!raw) return [];
...
function normalizeProperties(raw: ExtractedPolicyData['properties']): CanonicalProperty[] {
  if (!raw) return [];
...
function normalizeMortgagees(raw: ExtractedPolicyData['mortgagees']): CanonicalMortgagee[] {
  if (!raw) return [];
...
function normalizeDiscounts(raw: ExtractedPolicyData['discounts']): CanonicalDiscount[] {
  if (!raw) return [];
...
function normalizeLOB(lob: string | undefined): LineOfBusiness | undefined {
