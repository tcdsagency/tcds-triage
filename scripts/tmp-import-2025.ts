...
async function main() {
  ...
  console.log(`Parsed ${records.length} rows, ${headers.length} headers`);
  ...
  // Step 2: Field mapping (same as the 1-row test)
  ...
  for (let i = 0; i < records.length; i++) {
    ...
    if (i % 500 === 0 && i > 0) {
      ...
      console.log(`  Processing row ${i}/${records.length} (${elapsed}s) — imported=${importedRows} dupes=${duplicateRows} errors=${errorRows}`);
    }
