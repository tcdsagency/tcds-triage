...
function buildPropertyEntities(profile: MergedProfile): GayaEntity[] {
  ...
  for (const policy of profile.policies) {
    ...
    // HawkSoft uses codes: DWELL, PP, PL, LOU, OS, MEDPM, RPDED (deductible)
    if (policy.coverages?.length) {
      for (const cov of policy.coverages) {
        const covType = (cov.type || cov.description || '').toLowerCase();
        // Map coverage types to Gaya field names (includes HawkSoft codes)
        if (covType.includes('dwelling') || covType === 'dwell' || covType === 'cov a') {
          addField(fields, 'dwelling_coverage', cov.limit);
        } else if (covType.includes('personal property') || covType === 'pp' || covType === 'cov c' || covType === 'pers') {
          addField(fields, 'personal_property_coverage', cov.limit);
        } else if (covType.includes('liability') || covType === 'pl' || covType === 'cov e' || covType === 'liab') {
          addField(fields, 'personal_liability', cov.limit);
        } else if (covType.includes('loss of use') || covType === 'lou' || covType === 'cov d' || covType === 'loss') {
          addField(fields, 'loss_of_use', cov.limit);
        } else if (covType.includes('medical') || covType === 'medpm' || covType === 'cov f' || covType === 'medc') {
          addField(fields, 'medical_payments', cov.limit);
        } else if (covType.includes('other structures') || covType === 'os' || covType === 'cov b') {
          addField(fields, 'other_structures', cov.limit);
        }
        // Add deductible if present - HawkSoft uses RPDED or on dwelling
        if ((covType.includes('dwelling') || covType === 'dwell' || covType === 'rpded' || covType.includes('all other') || covType === 'aop') && cov.deductible) {
